...
def main():
    ...
    with open(os.path.join(sys.path[0], "user.bin"), "rb+") as f:
        input_bin = f.read()
        fw_bytes = f
        # Determine payload size / checksum locations from total BIN length
        # Clarity full flash BIN = 0x50000 (0x4000 boot area + 0x4c000 payload)
        if len(input_bin) == 0x50000:
            data_size = 0x4c000
            checksum_offsets = [(0, 0x4bf80), (1, 0x4bffe)]  # 0x4FF80/0x4FFFE minus 0x4000
        elif len(input_bin) == 393216:  # 0x60000
            data_size = 0x4c000
            checksum_offsets = [(0, 0x4bf80), (1, 0x4bffe)]
        elif len(input_bin) == 524288:  # 0x80000
            data_size = 0x6c000
            checksum_offsets = [(0, 0x6bf80), (1, 0x6bffe)]
        else:
            print(f"unsupported fw length: {len(input_bin)} bytes (0x{len(input_bin):x})")
            sys.exit()
        ...
        if input_bin_hash == '79b695a73fd5ff22cbfeb4b83908ab29': #CR-V thanks to joe1
            ...
            data_new = [
                ...
                '0x0000, 0x0917, 0x0DC5, 0x1017, 0x119F, 0x140B, 0x1680, 0x74BF, 0x7FFF',  # torque_table row 1 -- CUSTOM 32,767 MAX TORQUE (5.68871527778X STOCK)
                '0x0000, 0x0917, 0x0DC5, 0x1017, 0x119F, 0x140B, 0x1680, 0x74BF, 0x7FFF',  # torque_table row 2
                '0x0000, 0x0917, 0x0DC5, 0x1017, 0x119F, 0x140B, 0x1680, 0x74BF, 0x7FFF',  # torque_table row 3
                '0x0000, 0x0917, 0x0DC5, 0x1017, 0x119F, 0x140B, 0x1680, 0x74BF, 0x7FFF',  # torque_table row 4
                '0x0000, 0x0917, 0x0DC5, 0x1017, 0x119F, 0x140B, 0x1680, 0x74BF, 0x7FFF',  # torque_table row 5
                '0x0000, 0x0917, 0x0DC5, 0x1017, 0x119F, 0x140B, 0x1680, 0x74BF, 0x7FFF',  # torque_table row 6
                '0x0000, 0x0917, 0x0DC5, 0x1017, 0x119F, 0x140B, 0x2CB7, 0x74BF, 0x7FFF',  # torque_table row 7

                '0x009F, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x05DC, 0x05DC', # filter_table row 1
                '0x009F, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x05DC, 0x05DC', # filter_table row 2
                '0x009F, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x05DC, 0x05DC', # filter_table row 3
                '0x009F, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x05DC, 0x05DC', # filter_table row 4
                '0x009F, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x05DC, 0x05DC', # filter_table row 5
                '0x009F, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x05DC, 0x05DC', # filter_table row 6
                '0x009F, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x05DC, 0x05DC', # filter_table row 7
                ]
            data_label = [
                'speed_clamp_lo',
        ...
        elif input_bin_hash == 'd392cd7cf1173e43f3a1d1f4e31523e3':
            ...
            data_new = [
                ...
                '0x0000, 0x07DE, 0x1444, 0x2355, 0x32CC, 0x4266, 0x5577, 0x7732, 0x7FFF', # new torque_table row 1
                '0x0000, 0x07DE, 0x1444, 0x2355, 0x32CC, 0x4266, 0x5577, 0x7732, 0x7FFF', # new torque_table row 2
                '0x0000, 0x07DE, 0x1444, 0x2355, 0x32CC, 0x4266, 0x5577, 0x7732, 0x7FFF', # new torque_table row 3
                '0x0000, 0x07DE, 0x1444, 0x2355, 0x32CC, 0x4266, 0x5577, 0x7732, 0x7FFF', # new torque_table row 4
                '0x0000, 0x07DE, 0x1444, 0x2355, 0x32CC, 0x4266, 0x5577, 0x7732, 0x7FFF', # new torque_table row 5
                '0x0000, 0x07DE, 0x1444, 0x2355, 0x32CC, 0x4266, 0x5577, 0x7732, 0x7FFF', # new torque_table row 6
                '0x0000, 0x07DE, 0x1444, 0x2355, 0x32CC, 0x4266, 0x5577, 0x7732, 0x7FFF', # new torque_table row 7
            ...
            data_offsets = [
                ...
                0x1379a, #torque_table row 1
                0x137ac, #torque_table row 2
                0x137be, #torque_table row 3 <--- My car uses config index 3. The other rows don't matter, but they exist for some reason.
                0x137d0, #torque_table row 4
                0x137e2, #torque_table row 5
                0x137f4, #torque_table row 6
                0x13806, #torque_table row 7
            ...
            data_old = [
                ...
                '0x0000, 0x0917, 0x0DC5, 0x1017, 0x119f, 0x140b, 0x1680, 0x1680, 0x1680', # torque_table row 1
                '0x0000, 0x0917, 0x0DC5, 0x1017, 0x119f, 0x140b, 0x1680, 0x1680, 0x1680', # torque_table row 2
                '0x0000, 0x0917, 0x0DC5, 0x1017, 0x119f, 0x140b, 0x1680, 0x1680, 0x1680', # torque_table row 3 # My car verified
                '0x0000, 0x0917, 0x0DC5, 0x1017, 0x119f, 0x140b, 0x1680, 0x1680, 0x1680', # torque_table row 4
                '0x0000, 0x0917, 0x0DC5, 0x1017, 0x119f, 0x140b, 0x1680, 0x1680, 0x1680', # torque_table row 5
                '0x0000, 0x0917, 0x0DC5, 0x1017, 0x119f, 0x140b, 0x1680, 0x1680, 0x1680', # torque_table row 6
                '0x0000, 0x06b3, 0x0b1a, 0x0ccd, 0x0e9a, 0x104d, 0x119a, 0x11da, 0x11da', # torque_table row 7
                ...
                '0x009f, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108', # filter_table row 1
                '0x009f, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108', # filter_table row 2
                '0x009f, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108', # filter_table row 3 # My car verified
                '0x009f, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108', # filter_table row 4
                '0x009f, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108', # filter_table row 5
                '0x009f, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108', # filter_table row 6
                '0x009f, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108, 0x0108', # filter_table row 7
            ...
            data_new = [
                ...
                '0x0000, 0x0888, 0x1111, 0x1999, 0x2222, 0x3333, 0x4444, 0x5555, 0x7FFF', # new torque_table row 1
                '0x0000, 0x0888, 0x1111, 0x1999, 0x2222, 0x3333, 0x4444, 0x5555, 0x7FFF', # new torque_table row 2
                '0x0000, 0x0888, 0x1111, 0x1999, 0x2222, 0x3333, 0x4444, 0x5555, 0x7FFF', # new torque_table row 3
                '0x0000, 0x0888, 0x1111, 0x1999, 0x2222, 0x3333, 0x4444, 0x5555, 0x7FFF', # new torque_table row 4
                '0x0000, 0x0888, 0x1111, 0x1999, 0x2222, 0x3333, 0x4444, 0x5555, 0x7FFF', # new torque_table row 5
                '0x0000, 0x0888, 0x1111, 0x1999, 0x2222, 0x3333, 0x4444, 0x5555, 0x7FFF', # new torque_table row 6
                '0x0000, 0x0888, 0x1111, 0x1999, 0x2222, 0x3333, 0x4444, 0x5555, 0x7FFF', # new torque_table row 7
