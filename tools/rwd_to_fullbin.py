...
def parse_x5a_rwd(rwd: bytes):
    ...
    enc = rwd[i:]
    if len(enc) != length:
        raise RuntimeError(f"Encrypted payload length {len(enc)} != header length {length}")

    return key, start, length, enc
...
def main():
    ...
    key, start, length, enc = parse_x5a_rwd(rwd_path.read_bytes())
